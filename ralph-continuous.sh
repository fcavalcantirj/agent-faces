...
# Adapted from solvr/ralph-continuous.sh.
#
#   ./ralph-continuous.sh
#   BATCH_SIZE=3 BATCH_PAUSE_MINS=15 WAIT_TIME_MINS=15 ./ralph-continuous.sh
#   RALPH_PUSH=0 ./ralph-continuous.sh        # commit locally, do not push
# ─────────────────────────────────────────────────────────────────────────────
...
export PRD_FILE="${PRD_FILE:-prd.json}"
...
while true; do
  ...
  echo ""
  echo -e "${CYAN}${BOLD}┌───────────────────────────────────────────────────────────────────┐${NC}"
  echo -e "${CYAN}${BOLD}│  ▶ BATCH #${batch_count}   📅 $(date '+%Y-%m-%d %H:%M:%S')   🔄 ${BATCH_SIZE} iterations${NC}"
  echo -e "${CYAN}${BOLD}└───────────────────────────────────────────────────────────────────┘${NC}"
  echo ""
...
🔄 Running ${BATCH_SIZE} iterations..."
  ...
  # SUCCESS first — skip error detection if a run reported success.
...
⏸️ Next batch in ${BATCH_PAUSE_MINS} min"
  ...
  if [ "$prd_complete" = true ]; then
    ...
    echo -e "${GREEN}${BOLD}║   🎉  CLAUDE-FACES PRD COMPLETE!                                   ║${NC}"
